...
- `scripts/setup-vps.sh`: instalación de Docker desde el repositorio de la **distro detectada** (`/etc/os-release` → `linux/ubuntu` o `linux/debian`). Antes hardcodeaba Debian y rompía el bootstrap en Ubuntu (Ampere A1) con 404 en `apt-get update`. Añade la instalación explícita de `git` (las imágenes Ubuntu Minimal no lo incluyen).
