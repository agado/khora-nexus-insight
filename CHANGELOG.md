...
- RAG (fix): `execute_query` ahora respeta `is_public` al construir el contexto, igual que `document_service` — antes el contexto RAG solo obtenía documentos del propio departamento, incoherente con el listado y con la funcionalidad H7.2 de documentos compartidos. Para no duplicar la regla de acceso se extra `document_access_condition()` (departamentos propios U public) como fuente única usada por `execute_query`, `get_document_by_id` y `get_documents_by_departments`, y la query queda en una sola sentencia (el filtro de `document_ids` se encadena). +2 tests del WHERE real del statement (274 total).
- Seguridad (Zero-Trust): `delete_document` deja de reutilizar `get_document_by_id` (que incluye `is_public` por ser una operación de lectura compartida) y usa ahora un alcance estricto por `department_id` — un documento público de otro departamento se puede LEER pero nunca BORRAR por quien no pertenece a ese departamento. +2 tests del servicio (276 total).
